...
import { hostConnectionKey, sameHostConnection, type HostScopeToken } from "../features/shell/hostScope";
...
export function useTmuxActionPerformer(options: Options) {
  return useCallback(async (
    ...
    const paneSpanHandle = paneSpan ? openPerfSpan(paneSpan, hostConnectionKey(initialScope)) : undefined;
    try {
      ...
      if (!sameHostConnection(initialScope, options.hostScopeRef.current)) {
        if (paneSpan) abandonPerfSpan(paneSpan, paneSpanHandle);
        return undefined;
      }
