...
describe("TauriAgentClient", () => {
  ...
  it("maps the protobuf-derived JSON snapshot into the adapter-neutral record", async () => {
    ...
    expect(snapshot).toMatchObject({ revision: "6", acceptedGeneration: "6", notificationWatermark: "4", authoritative: true, adapters: [{ id: "future", displayName: "Future", placements: ["window", "split"], supportsProcessDetection: true }], agents: [{ id: "agent-1", adapterId: "codex", lifecycle: "blocked", authority: "hook", attentionGeneration: "4", attentionKind: "blocked", paneId: "%1", detectedManually: true }] });
  ...
  it("requires a host-issued confirmation token before hook install or uninstall", async () => {
    invokeMock.mockResolvedValueOnce({ hookPlan: { adapter: "claudeCode", adapterId: "claude-code", action: "review", configPath: "/config", backupPath: "/backup", managedVersion: "tmux-agent-ide/v1", summary: "one managed hook", confirmationToken: "confirm", alreadyCurrent: false, ownershipMarker: "Claude adapter", proposedCommand: "/usr/bin/host hook ingest", proposedEvents: ["Stop"], trustGuidance: "Review trust", beforeHash: "old", afterHash: "new", createsConfig: false, removesConfig: true, beforePreview: "{\n  redacted\n}", afterPreview: "{\n  managed\n}", diffPreview: "--- before\n-redacted\n+++ after\n+managed", previewTruncated: false } }).mockResolvedValueOnce({});
    const client = new TauriAgentClient();
