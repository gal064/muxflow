...
fn phase14_git_measurements() -> Option<&'static Mutex<GitPhase14Measurements>> {
    static VALUE: OnceLock<Mutex<GitPhase14Measurements>> = OnceLock::new();
    if !cfg!(test) && std::env::var_os("ADE_PHASE14_HOST_METRICS").is_none() {
        return None;
    }
    Some(VALUE.get_or_init(|| Mutex::new(GitPhase14Measurements::default())))
}

pub(super) fn phase14_git_watcher_created() {
    let Some(value) = phase14_git_measurements() else {
        return;
    };
...
pub(super) fn phase14_git_process_started(command: &OsStr) -> Option<GitProcessMeasurement> {
    let value = phase14_git_measurements()?;
    let mut value = value.lock().unwrap();
    ...
    Some(GitProcessMeasurement)
}

impl Drop for GitProcessMeasurement {
    fn drop(&mut self) {
        if let Some(value) = phase14_git_measurements() {
            let mut value = value.lock().unwrap();
...
pub(super) fn phase14_git_snapshot() -> GitPhase14Measurements {
    phase14_git_measurements().unwrap().lock().unwrap().clone()
}
