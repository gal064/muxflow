...
impl FileService {
    ...
    pub(crate) fn open_file_stream(
        ...
        self.open_file_stream_authorized(root, &token, path)
    }
    ...
    pub(crate) fn open_file_stream_authorized(
        ...
        let (kind, content) = if image {
            if size <= MAX_IMAGE_BYTES {
                (v1::FileContentKind::Image, read_bounded(&mut file, size)?)
            } else {
            ...
            let bytes = read_bounded(&mut file, size)?;
        ...
        if metadata_generation(&opened_before) != metadata_generation(&opened_after) {
            bail!("stale_file_generation: file changed while it was being opened");
        }
...
fn read_bounded(file: &mut File, expected: u64) -> anyhow::Result<Vec<u8>> {
