...
[[ $(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$target/Contents/Info.plist") == dev.dev.tmux-agent-ide ]]

host="$target/Contents/MacOS/tmux-ide-host"
if [[ -x "$host" ]]; then
...
umask 077
quarantine=$(mktemp -d "$applications/.tmux-agent-ide.uninstall.XXXXXX")
mv "$target" "$quarantine/$name"
rm -rf "$quarantine"
printf 'Removed tmux Agent IDE. tmux sessions and user configuration were preserved.\n'
